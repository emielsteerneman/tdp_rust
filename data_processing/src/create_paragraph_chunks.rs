...
impl Recreate for Vec<Chunk> {
    ...
    fn raw(&self) -> Vec<String> {
        self.iter().map(|chunk| chunk.text.clone()).collect()
    }
}
...
fn trim_duplicates_and_subsets(chunks: Vec<Vec<usize>>) -> Vec<Vec<usize>> {
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        // println!(" trim chunk: {:?}", chunk);

        // Check if chunk is a subset of any other chunk in the original list (excluding itself)
        let is_subset = chunks
            .iter()
            .enumerate()
            .any(|(j, other_chunk)| i != j && chunk.iter().all(|item| other_chunk.contains(item)));

        if !is_subset {
            trimmed_chunks.push(chunk.clone());
        }
    }
